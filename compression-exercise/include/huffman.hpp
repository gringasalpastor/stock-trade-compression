...
/*
 * Collection of functions and structs for creating Huffman encodings. 
 * All functions are templated based on the Symbol type, as to make it easy to switch from different symbols.
 * For example, you could change the side from a 'char' to a 'string' with very little code changes.
 */
...
{       
    namespace huffman
    {
        ...
        namespace detail
        {
             ...
             */
            template<class Symbol>
            void traverse_nodes(const std::shared_ptr<node<Symbol>> node,
                                std::shared_ptr<std::map<Symbol, std::vector<bool>>> prefix_map,
                                const std::vector<bool>& previous_prefix = {})
            {
                ...
                if(node->left)
                    traverse_nodes(node->left, prefix_map, new_left);
                if(node->right)
                    traverse_nodes(node->right, prefix_map, new_right);
            }
            ...
            {
                /*
                 * Proper comparison for canonical Huffman tree construction is first length, then symbol order.
                 * Your Symbol type must be comparable
                 */
        ...
        template<class Symbol>
        std::shared_ptr<std::map<Symbol, long>> make_count_map()
        {
            return std::make_shared<std::map<Symbol, long>>();
        }
         ...
         */
        template<class Symbol>
        std::shared_ptr<std::map<Symbol, std::vector<bool>>> create_prefix_map(const std::shared_ptr<node<Symbol>> root)
        {            
            auto prefix_map = std::make_shared<std::map<Symbol, std::vector<bool>>>();
         ...
         */
        template<class Symbol>
        std::shared_ptr<std::map<Symbol, std::vector<bool>>> create_canonical_prefix_map(
            const std::set<symbol_data<Symbol>, detail::symbol_data_less_f<Symbol>>& symbol_data_)
        {
            auto canonical_prefix_map = std::make_shared<std::map<Symbol, std::vector<bool>>>();
            std::vector<bool> canonical_prefix;
            for(const auto& sym : symbol_data_)
            {
                while(canonical_prefix.size() < sym.prefix_length)
                    canonical_prefix.push_back(false);
        ...
        template<class Symbol>
        std::shared_ptr<std::map<Symbol, std::vector<bool>>> create_canonical_prefix_map(
            const std::shared_ptr<std::map<Symbol, std::vector<bool>>> prefix_map)
        {
         ...
         */
        template<class Symbol>
        std::shared_ptr<node<Symbol>> make_huffman_tree(const std::shared_ptr<std::map<Symbol, long>> symbol_count, const uint64_t num_trades)
        {
            std::priority_queue<std::shared_ptr<node<Symbol>>,
                                    std::vector<std::shared_ptr<node<Symbol>>>,
                                    detail::node_less_f<Symbol>> symbol_queue;
