#ifndef COMPRESS_INTERPOLATE_H
#define COMPRESS_INTERPOLATE_H
...
/*
 * Uses Newton's Divided-Difference algorithm to interpolate `x_interpolate` for
 * the given input ranges `x_values` and `y_values`. See Numerical
 * Analysis(Burden & Faires) page 121
 */
...
double newton_interpolate(const XRange& x_values, const YRange& y_values, const double x_interpolate) {
  ...
  for (const auto i : boost::irange(1, size)) {
    std::ignore = i;
  ...
  // Calculate the sum of the products; See Numerical Analysis(Burden & Faires)
  // as reference
  double sum = 0;
...
double predict_next_value(const YRange& y_values) {
  ...
  // If only one value predict that value, else interpolate with x values as
  // 0,1,2,3,4...
