...
trade parse_trade(const std::string& line, const char new_line) {
  ...
  unsigned p_index = 0;
  while (p_index < price_token.size() and price_token[p_index] != '.') price_string.push_back(price_token[p_index++]);
  ...
  if (price_token[0] == '-') t.overflow_price_digits.push_back('-');
  /*
   * Copy all of the extra digits to overflow_price_digits. Also stores the '-'
   * Example "-3.1415" --> "-15"
   */
  auto index = price_token.find('.');
  if (index != std::string::npos) {
    index += 3;
    if (index < price_token.size()) t.overflow_price_digits += price_token.substr(index);
  }
...
boost::iterator_range<detail::csv_source_iterator> make_csv_source_range(const boost::filesystem::path& path) {
  return boost::make_iterator_range(detail::csv_source_iterator(std::make_shared<std::ifstream>(
                                        path.string(), std::ios_base::in | std::ios_base::binary)),
                                    detail::csv_source_iterator());
}

namespace detail {
csv_source_iterator::csv_source_iterator(std::shared_ptr<std::ifstream> stream_) : stream(stream_) { parse_line(); }
...
void csv_source_iterator::parse_line() {
  if (std::getline(*stream, line, '\n')) {
    line_ending = '\n';
    if (not line.empty() and line.back() == '\r') line_ending = '\r';
    boost::trim(line);
...
void csv_source_iterator::increment() { parse_line(); }

bool csv_source_iterator::equal(csv_source_iterator const& other) const {
  return this->stream == nullptr and other.stream == nullptr;
}

trade csv_source_iterator::dereference() const { return parse_trade(line, line_ending); }
}  // namespace detail
