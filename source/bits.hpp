#ifndef COMPRESS_BITS_H
#define COMPRESS_BITS_H
...
namespace compress {
/*
 * bits_to_bytes_converter takes in multiple sequences of bits and packs them into a bytes. bytes are stored in a
 * `std::vector<uint8_t>`.
 */
struct bits_to_bytes_converter : boost::noncopyable {
  ...
  /*
   * Pack all of the bits in `bits` into a vector of bytes. The final byte is padded out with 0's if the number of bits
   * mod 8 != 0
   */
  void append_bits(const std::vector<bool>& bits) {
    for (std::size_t i = 0; i < bits.size(); i++) {
      ...
      if (byte_index >= bytes.size()) bytes.push_back(0);
  ...
  /*
   * Removes all fully packed bytes. If the final byte is not fully packed it will still remain, and `data().size() ==
   * 1`, else `data().size() == 0`.
   */
   ...
   */
  unsigned long number_of_fully_packed_bytes() { return (bit_index / 8); }
...
void increment_bits(std::vector<bool>& bits) {
  ...
  if (carry) bits.insert(boost::begin(bits), true);
}
