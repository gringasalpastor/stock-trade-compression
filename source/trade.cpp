...
namespace exercise
{
    ...
    std::string trade::to_csv() const
    {
        ...
        while(decimal_part.size() < 2)
            decimal_part.insert(boost::begin(decimal_part), '0');
        
        decimal_part = "." + decimal_part + local_overflow_price_digits;
        
        //Remove trailing 0's
        while(decimal_part[decimal_part.size() - 1] == '0')
        ...
        ss << boost::format(format)
            % symbol % exchange_new_line.first % side % condition % send_time % receive_time % 
            (minus_str + integer_part_str + decimal_part) % quantity;
            
        /*
        * Don't use the system new lines, we could have a mix of new line types in the file
        * NOTE: '\r' really means "\r\n"
        */ 
