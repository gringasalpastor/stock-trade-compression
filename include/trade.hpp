...
namespace exercise
{
     ...
     * 'price' - Stores price as price * 100 truncated to 2 decimal places ('3.1415' --> '314')
     * 'overflow_price_digits' - Stores the remaining truncated decimal places from the price value
     *      examples: '3.1415' --> '15'  '3.14' --> ''(empty string)
     */
    struct trade
    {
        std::string symbol;                 // Variable-length string of chars
        /* 
         * Stores a pair of the newline and the exchange.
         * The newline for a given trade is ('\n' = Unix, '\r' = Windows)
         */
        ...
        int64_t receive_time;               // Time at which the event was received on the local host (milliseconds-after-midnight)
     ...
     */
    trade make_trade(const std::string& symbol_,
                     const std::pair<char, char>& exchange_new_line_,
                     const char side_,
                     const char condition_,
                     const int64_t send_time_,
                     const int64_t receive_time_,
                     const int32_t price_,
                     const std::string& overflow_price_digits_,
                     const int32_t quantity_);
